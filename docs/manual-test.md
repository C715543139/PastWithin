...
2. 确认 popup 正常显示搜索框、搜索方式菜单、搜索按钮、帮助按钮和设置按钮。
3. 确认搜索框、搜索方式菜单和搜索按钮在同一行且高度一致。
4. 默认保持“分词”，输入关键词后停止输入约 300ms，确认会自动发起分词查询。
5. 连续快速输入多个字符，确认不会每个字符都刷新结果，最后展示完整输入对应的结果。
6. 使用中文输入法输入中文词，确认拼音组合输入过程中不会提前搜索，选词完成后再自动搜索。
7. 切换全文搜索，确认页面显示“按 Enter 或点击搜索”一类提示。
8. 在全文搜索下输入连续片段，确认不会实时搜索；按 Enter 或点击“搜索”后才发起全文查询。
9. 在全文搜索下输入 `R2` 或 `tf` 这类长度小于等于 2 的短查询，确认会先提示可能较慢；点击“继续全文搜索”后才发起查询，点击“取消”不会搜索。
10. 关闭“保存正文”后，确认全文搜索不可选择。
11. 首次分词查询可能触发 `jieba-wasm` 初始化；确认首次搜索不会报错，稍后重复搜索响应正常。
12. 确认搜索结果显示圆形浅灰底 favicon 图标；图标获取失败时显示通用地球图标；书签页面在顶部区域右侧用“已收藏”徽标标记，标题和摘要位于其下方，摘要过长时不会超过三行。
